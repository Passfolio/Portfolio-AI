...
사용법:
  ...
  python scripts/test_local.py cl_improve  --pdf pdfsample/펄어비스_웹개발(백엔드).pdf --code scripts/final-3.json
  ...
  python scripts/test_local.py cl_to_pf    --pdf pdfsample/펄어비스_웹개발(백엔드).pdf --job 백엔드 --career 2년 --code scripts/final-3.json
...
옵션:
  ...
  --code     코드 분석 JSON 파일 경로 (없으면 RAG만, 있으면 RAG+코드분석 강화)
"""
...
def _load_code(code: Path | None) -> dict | None:
    if code is None:
        return None
...
# ── 1. 자소서 개선 ────────────────────────────────────────────────
def test_cl_improve(pdf: Path, no_rag: bool = False, code: Path | None = None, **_):
    code_analysis = _load_code(code)
    mode = "프롬프트 전용" if no_rag else ("RAG+코드분석" if code_analysis else "RAG")
    ...
    try:
        with patch("app.services.cover_letter.download_pdf_to_temp", return_value=tmp), \
            ...
            result = run_cover_letter_from_pdf(
                pdf_s3_url="local", use_rag=not no_rag, code_analysis=code_analysis,
            )
        _save(result, "cl_improve")
...
# ── 3. 포트폴리오 개선 ────────────────────────────────────────────
def test_pf_improve(pdf: Path, code: Path | None = None, **_):
    code_analysis = _load_code(code)
    mode = "RAG+코드분석" if code_analysis else "RAG"
    print(f"\n[포트폴리오 개선 — {mode}] {pdf.name}")
    ...
    try:
        with patch("app.services.portfolio.download_pdf_to_temp", return_value=tmp), \
            ...
            result = run_portfolio_from_pdf(pdf_s3_url="local", code_analysis=code_analysis)
...
# ── 4. 자소서 → 포트폴리오 생성 ──────────────────────────────────
def test_cl_to_pf(pdf: Path, job: str | None, career: str | None, code: Path | None = None, **_):
    from app.services._rag_utils import map_career_input
    career_mapped = map_career_input(career) if career else None
    code_analysis = _load_code(code)
    mode = "RAG+코드분석" if code_analysis else "RAG"
    print(f"\n[자소서 → 포트폴리오 생성 — {mode}] {pdf.name}")
    print(f"  job={job or '미지정'}  career={career or '미지정'}" +
          (f" → {career_mapped}" if career_mapped else ""))
    ...
    try:
        with patch("app.services.cover_letter.download_pdf_to_temp", return_value=tmp), \
            ...
            result = run_cover_letter_to_portfolio(
                pdf_s3_url="local", job=job, career=career_mapped, code_analysis=code_analysis,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--code",   help="코드 분석 JSON 파일 경로 (없으면 RAG만, 있으면 RAG+코드분석 강화)")
